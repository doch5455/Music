#
# Telif Hakkı (C) 2021-2023 ArchBots@Github, < https://github.com/ArchBots >.
#
# Bu dosya < https://github.com/ArchBots/ArchMusic > projesinin bir parçasıdır,
# ve "GNU v3.0 Lisans Anlaşması" altında yayınlanmıştır.
# Lütfen bakınız < https://github.com/ArchBots/ArchMusic/blob/master/LICENSE >
#
...
async def gstats_global(client, message: Message, _):
    ...
    title = title.title()
    final = f"**{MUSIC_BOT_NAME}** üzerinde En Çok Çalınan Parça\n\n**Başlık:** {title}\n\n**{co}** kez çalındı"
...
async def top_users_ten(client, CallbackQuery: CallbackQuery, _):
    ...
    mystic = await CallbackQuery.edit_message_text(
        _["gstats_3"].format(
            f"{CallbackQuery.message.chat.title} İçin"
            if what == "Here"
            else what
        )
    )
    ...
    def get_stats():
        ...
        if what in ["Tracks", "Here"]:
            for items, count in list_arranged.items():
                ...
                if items == "telegram":
                    msg += f"🔗[Telegram Dosyaları ve Sesleri](https://t.me/telegram) **{count} kez oynatıldı**\n\n"
                else:
                    msg += f"🔗 [{title}](https://www.youtube.com/watch?v={items}) **{count} kez oynatıldı**\n\n"
    ...
    if what in ["Users", "Chats"]:
        for items, count in list_arranged.items():
            ...
            try:
                ...
                continue
            limit += 1
            msg += f"🔗`{extract}` botta {count} kez oynatıldı.\n\n"
        temp = (
...
**Toplam Sorgular:** {total_queries}
**Toplam Asistanlar:** {assistant}
**Otomatik Ayrılan Asistan:** {ass}
**Temizleme Modu Süresi:** {cm} Dakika

**Oynatma Süresi Sınırı:** {play_duration} Dakika
**Şarkı İndirme Sınırı:** {song} Dakika
**Botun Sunucu Çalma Listesi Sınırı:** {playlist_limit}
